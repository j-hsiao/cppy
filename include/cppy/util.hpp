...
namespace cppy
{
	...
	//Type of *arg, or void if *arg is not valid.
	template<class T>
	decltype(*std::declval<T&&>()) startype(T &&t);
	int startype(...);

	//*obj is a T
	//is a struct
	template<class T, class Actual>
	decltype(is<T>(*std::declval<Actual&&>())) star_is(Actual&&);
	template<class T> std::false_type star_is(...);
	//is a template
	template<template<class...>class T, class Actual>
	decltype(is<T>(*std::declval<Actual&&>())) star_is(Actual&&);
	...
	template<class T, class Actual> struct cvrefmatch<T, Actual&&> { typedef T&& type; };
	template<class T, class Actual> struct cvrefmatch<T, const Actual> { typedef const T type; };
	template<class T, class Actual> struct cvrefmatch<T, const Actual&> { typedef const T& type; };
	template<class T, class Actual> struct cvrefmatch<T, const Actual&&> { typedef const T&& type; };
	template<class T, class Actual> struct cvrefmatch<T, volatile Actual> { typedef volatile T type; };
	template<class T, class Actual> struct cvrefmatch<T, volatile Actual&> { typedef volatile T& type; };
	template<class T, class Actual> struct cvrefmatch<T, volatile Actual&&> { typedef volatile T&& type; };

	//Convert to T with same cvref qualifiers if possible
	//Otherwise, return same as input.
	template<class T, class Actual>
